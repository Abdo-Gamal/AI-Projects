...
class Linear(object):
    ...
    def backward(dout, cache):
        ...
        ##################################################
        # Replace "pass" statement with your code
        #genral  rule  dowenstream = upersteam *localstream
        # ex           d (loss/ w) = d (loss/ out)  * d( out/ w) 
        # out=x @ w+b
        dw=x_row.T @ dout      # X(N,D) ,dout  (N,m)-->(D,M)
        dx= dout @ w.T     #dout(n,m), w (D,m) ---->(N,D)
        db=dout.sum(dim=0)  
...
class TwoLayerNet(object):
    ...
    def __init__(self, input_dim=3*32*32, hidden_dim=100, num_classes=10,
        ...
        ###################################################################
        # Replace "pass" statement with your code
        Din1=input_dim         # 3*32*32
        Dout1=hidden_dim       #100
        Dout2=num_classes

        self.params['W1']=torch.randn(Din1,Dout1,dtype=dtype,device=device)*weight_scale #/torch.sqrt(Din1)
        self.params['b1']=torch.zeros(Dout1,dtype=dtype,device=device)
        
        self.params['W2']=torch.randn(Dout1,Dout2,dtype=dtype,device=device)*weight_scale#/torch.sqrt(Dout1)
        self.params['b2']=torch.zeros(Dout2,dtype=dtype,device=device)
    ...
    def loss(self, X, y=None):
        ...
        N=X.shape[0]
        x_row=X.reshape(N,-1)
        out_relu, cache_relu = Linear_ReLU.forward(x_row, self.params['W1'], self.params['b1'])
        scores, cache = Linear.forward(out_relu, self.params['W2'], self.params['b2'])
        ############
        ...
        ###################################################################
        # Replace "pass" statement with your code
       # upstream_grad tells you how much each class scoreshould increase/decrease 
       #to reduce the loss.
        loss,upstream_grad= softmax_loss(scores ,y )
        loss+=self.reg*(torch.sum(self.params['W1']**2)+ torch.sum(self.params['W2']**2))
    
        dx2,dw2,db2=Linear.backward(upstream_grad,cache)
        grads['b2']=db2
        grads['W2']=dw2+2*self.reg*self.params['W2']
        dx1,dw1,db1=Linear_ReLU.backward(dx2,cache_relu)
        grads['b1']=db1
...
class FullyConnectedNet(object):
    ...
    def __init__(self, hidden_dims, input_dim=3*32*32, num_classes=10,
        ...
        # Replace "pass" statement with your code
        Din = input_dim
        # hidden_dims is a list with length L-1; total layers L = 1 + len(hidden_dims)
        for i, Dout in enumerate(hidden_dims, start=1):
            # i = 1,2,..., len(hidden_dims)
            self.params[f"W{i}"] = torch.randn(Din, Dout, dtype=dtype, device=device) * weight_scale
            self.params[f"b{i}"] = torch.zeros(Dout, dtype=dtype, device=device)
            Din = Dout  # next layer input dim is current output dim
        

        # final layer: from last hidden size (Din) to num_classes
        final_idx = self.num_layers
        self.params[f"W{final_idx}"] = torch.randn(Din, num_classes, dtype=dtype, device=device) * weight_scale
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        N=X.shape[0]
        input_row=X.reshape(N,-1)
        caches = []
        drop_caches=[]
        for i  in range(1,self.num_layers):
          W=self.params[f'W{i}']
          b= self.params[f'b{i}']
          out, cache_i = Linear_ReLU.forward(input_row, W,b)
          if self.use_dropout:
            out,drop_cache_i=Dropout.forward(out,self.dropout_param)
            drop_caches.append(drop_cache_i)
        ...
        ############
        final_idx = self.num_layers
        scores, cache = Linear.forward(input_row, self.params[f'W{final_idx}'], self.params[f'b{final_idx}'])
        caches.append(cache)
        #################################################################
        #                      END OF YOUR CODE                         #
        #################################################################
        ...
        final_idx = self.num_layers
        dx,dw,db=Linear.backward(upstream_grad,caches[-1])
        grads[f'W{final_idx}']=dw+2*self.reg*self.params[f'W{final_idx}']
        grads[f'b{final_idx}']=db

        for i  in range(final_idx-1,0,-1):

          if self.use_dropout:
            dx=Dropout.backward(dx,drop_caches[i-1])
          dx,dw,db=Linear_ReLU.backward(dx,caches[i-1])# caches start form zero not one like prams
          grads[f'b{i}']=db
...
def create_solver_instance(data_dict, dtype, device):
    ...
    #############################################################
    data = {
     'X_train': data_dict['X_train'],# training data
     'y_train': data_dict['y_train'],# training labels
     'X_val':data_dict['X_val'] ,# validation data
     'y_val':data_dict['y_val'], # validation labels
    }
    solver = None
    # Replace "pass" statement with your code
...
def rmsprop(w, dw, config=None):
    ...
    epc=config['epsilon']
    v=config['cache']
    alph=config['learning_rate']
    v=B*v+(1-B)*dw**2
    next_w=w-(alph/(torch.sqrt(v)+epc))*dw
...
def adam(w, dw, config=None):
    ...
    v_hat=v/(1-B2**t)
    next_w=w-(lr/(torch.sqrt(v_hat)+eps))*m_hat
...
class Dropout(object):
    ...
    def backward(dout, cache):
        ...
        if mode == 'train':
            ...
            ###########################################################
            # Replace "pass" statement with your code
            dx=dout*(mask>0)/(1-dropout_param['p'])
            ###########################################################
            #                     END OF YOUR CODE                    #
            ###########################################################
        elif mode == 'test':
