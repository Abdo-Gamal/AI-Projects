...
from torch.nn.modules import linear, pooling
...
class Conv(object):
    ...
    def forward(x, w, b, conv_param):
        ...
        out_hight=1 + (H + 2 * pad - HH) // stride
        out_width=1+(W+2*pad-WW)//stride
        out=torch.zeros((N,F,out_hight,out_width),dtype=x.dtype,device=x.device)

        x_pad=torch.nn.functional.pad(x,(pad,pad,pad,pad), mode='constant', value=0)#np.pad(x, ((0,0),(0,0),(pad,pad),(pad,pad)), mode='constant', constant_values=0)
          ...
          for f in range(F):
            for i in range(out_hight):
              for j in range(out_width):
                ...
                st_h=stride*i 
                st_w=stride*j
                img=x_pad[n,:,st_h:st_h+HH,st_w:st_w+WW]   #3*6*6
                out[n,f,i,j]=torch.sum(filter_n * img)+b[f]
    ...
    def backward(dout, cache):
        ...
        # init gradients
        dx = torch.zeros_like(x)# same divice and dtype
        dw = torch.zeros_like(w)
        db = torch.zeros_like(b)
      #calc dl/db
        db=torch.sum(dout,dim=(0,2,3))
      # calc  dl/dw not make identical of slide 
        x_pad=torch.nn.functional.pad(x,(pad,pad,pad,pad), mode='constant', value=0)#np.pad(x, ((0,0),(0,0),(pad,pad),(pad,pad)), mode='constant', constant_values=0)
        dx_pad=torch.nn.functional.pad(dx,(pad,pad,pad,pad), mode='constant', value=0)#np.pad(x, ((0,0),(0,0),(pad,pad),(pad,pad)), mode='constant', constant_values=0)
          ...
          for f in range(F):
            for i in range(out_hight):
              for j in range(out_width):
                st_h=stride*i
                st_w=stride*j
                x_patch=x_pad[n,:,st_h:st_h+HH,st_w:st_w+WW]   # (C, HH, WW)
                 # scalar upstream
                dout_val = dout[n, f, i, j]
                dw[f]+=dout_val * x_patch
                #dx   not use away in screan 
                dx_pad[n,:,st_h:st_h+HH,st_w:st_w+WW]+=dout_val*w[f]
        if pad!=0:
...
class MaxPool(object):
          ...
          for c in range(C):
            for i in range(out_h):
              for j in range(out_w):
                str_i=stride*i
                str_j=stride*j
                out[n,c,i,j]=torch.max(x[n,c,str_i:str_i+HH,str_j:str_j+WW])
    ...
    def backward(dout, cache):
        ...
        # Replace "pass" statement with your code
        x,pool_param=cache
        stride,pool_h,pool_w=pool_param['stride'],pool_param['pool_height'],pool_param['pool_width']
          ...
          for c in range(C):
            for i in range(out_h):
              for j in range(out_w):
                i0=stride*i
                j0=stride*j
                wind=x[n,c,i0:i0+pool_h,j0:j0+pool_h]   # shape (pool_h, pool_w)
                mx_val=torch.max(wind)
                mask=(wind==mx_val)
                dx[n,c,i0:i0+pool_h,j0:j0+pool_w]+=mask*dout[n,c,i,j]
...
class ThreeLayerConvNet(object):
    ...
    def __init__(self,
        ...
        # hidden affine layer input dim
        H_pool=H//2
        W_pool=W//2
        hidden_input_dim = num_filters* H_pool * W_pool
        self.params['W2']= torch.randn((hidden_input_dim,hidden_dim),dtype=dtype,device=device)*weight_scale
    ...
    def loss(self, X, y=None):
        ...
        # Replace "pass" statement with your code
        #conve layer
        out_conve,cache_conv=Conv_ReLU_Pool.forward(X,W1,b1,conv_param,pool_param)
        #linear layer
        N=X.shape[0]
        flat =out_conve.reshape(N,-1)
        out_hidden,fc1_cache =Linear_ReLU.forward(flat, W2,b2 )
        #last layer
        scores,fc2_cache=Linear.forward(out_hidden,W3,b3)
        #####################################################################
        #                             END OF YOUR CODE                       #
        ######################################################################
        ...
        loss_reg=torch.sum(W1**2)+ torch.sum(W2**2)+torch.sum(W3**2)
        loss=loss+self.reg*loss_reg
        ###
        fc2_dx,fc2_dw,fc2_db=Linear.backward(dx,fc2_cache)
        grads['b3']=fc2_db
        grads['W3']=fc2_dw+2*self.reg*self.params['W3']
        ###
        flat_dx,fc1_dw,fc1_db=Linear_ReLU.backward(fc2_dx,fc1_cache)
        grads['b2']=fc1_db
        grads['W2']=fc1_dw+2*self.reg*self.params['W2']
        ###
        conv_upstream=flat_dx.reshape(out_conve.shape)
        conv_dx,conv_dw,conv_db=Conv_ReLU_Pool.backward(conv_upstream,cache_conv)
        grads['b1']=conv_db
        grads['W1']=conv_dw+2*self.reg*self.params['W1']
...
class DeepConvNet(object):
    ...
    def __init__(self,
        ...
        C,H,W=input_dims
        in_channels=C
        Is_kaiming=(weight_scale=='kaiming')
       
        for  layer in range(1,self.num_layers):
          if Is_kaiming :
            weight_scale=math.sqrt(2.0/(in_channels*3*3))
            
          filter_i=num_filters[layer-1]
          self.params[f'W{layer}']=torch.randn((filter_i,in_channels,3,3),dtype=dtype,device=device)*weight_scale
          self.params[f'b{layer}']=torch.zeros((filter_i),dtype=dtype,device=device)
          in_channels=filter_i

        # NOW calculate the spatial dimensions AFTER all conv layers
        H_current, W_current = H, W
        for layer_idx in range(len(num_filters)):
           if layer_idx in self.max_pools:
              H_current = H_current // 2
              W_current = W_current // 2

        hidden_input_dim = num_filters[-1]* H_current * W_current   
        
        if Is_kaiming :
          weight_scale=math.sqrt(2.0/hidden_input_dim)
        
        final_idx=self.num_layers
        self.params[f'W{final_idx}']=torch.randn((hidden_input_dim,num_classes),dtype=dtype,device=device)*weight_scale
        self.params[f'b{final_idx}']=torch.zeros((num_classes),dtype=dtype,device=device)
    ...
    def loss(self, X, y=None):
        ...
        for i in range(1,self.num_layers):
          W=self.params[f'W{i}']
          b=self.params[f'b{i}']
          out,cache=Conv_ReLU.forward(input_i,W,b,conv_param)
          caches_conv_relu.append(cache)
          if (i-1) in self.max_pools:
            out,cache=FastMaxPool.forward(out,pool_param)
            caches_maxpool.append(cache)
          else :
            caches_maxpool.append(None)
          input_i=out
        
        final_idx=self.num_layers
        W=self.params[f'W{final_idx}']
        b=self.params[f'b{final_idx}']
        flat=out.reshape(N,-1)
        scores,cache_linear=Linear.forward(flat,W,b)
        #####################################################
        #                 END OF YOUR CODE                  #
        #####################################################
        ...
        loss=loss+self.reg *loss_reg
        flat_dx,dw,db=Linear.backward(dx,cache_linear)

        grads[f'W{final_idx}']=dw+2*self.reg*self.params[f'W{final_idx}']
        grads[f'b{final_idx}']=db
 
        dx = flat_dx.reshape(out.shape)
        for i in range(self.num_layers-1,0,-1):
          if caches_maxpool[i-1] is not None:
           dx=FastMaxPool.backward(dx,caches_maxpool[i-1])

          dx,dw,db=Conv_ReLU.backward(dx,caches_conv_relu[i-1])
...
class BatchNorm(object):
    ...
    def forward(x, gamma, beta, bn_param):
        ...
        if mode == 'train':
            ...
            ##################################################################
            # Replace "pass" statement with your code
            sample_mean=(1.0/N)*torch.sum(x,dim=0)   #shape(D,)
            sample_var=(1.0/N)*torch.sum((x-sample_mean)**2 ,dim=0 )    # x shape (N,D), mean_broadcast(N,D)
    ...
    def backward(dout, cache):
        ...
        #x_hat affect by x ,mean affect by x  , var affect by x ,mean
        dvar=torch.sum( dxhat*(x-sample_mean),dim=0)*(-1/2)*(sample_var+eps)**(-3/2)
        dmean1=torch.sum(dxhat*(-1/std),dim=0)
        dmean2=dvar*torch.sum(-2*(x-sample_mean)/N,dim=0)
        dmean=dmean1+dmean2

        dx=dxhat/std+dvar*2*(x-sample_mean)/N+dmean/N
