...
def hello():
    """
    This is sample function that we will try to import and run to ensure that
...
def compute_distances_two_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(0,num_train):
      vec1 = x_train[i].view(-1)
      for j in range(0,num_test):
        vec2 = x_test[j].view(-1)
        dist = ((vec1 - vec2) ** 2).sum()
        dists[i, j] = dist
...
def compute_distances_one_loop(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    for i in range(num_train):
      test_vecs = x_test.view(num_test, -1)  # (num_test, D)
      dist=((x_train[i].flatten()-test_vecs)**2).sum(dim=1)   #(D,) -(num_test,D)-->(num_test,D)
      dists[i,:]=dist
...
def compute_distances_no_loops(x_train: torch.Tensor, x_test: torch.Tensor):
    ...
    ##########################################################################
    # Replace "pass" statement with your code   applay this equation a**2 -2ab+b**2
    x_train_flat=x_train.view(num_train,-1)   # shape (num_train,1)
    x_test_flat=x_test.view(num_test,-1)     # shape (num_test,1)

    train_sq=(x_train_flat**2).sum(axis=1).reshape(num_train,1)
    test_sq=(x_test_flat**2).sum(axis=1).reshape(1,num_test)
    cross=  2*(x_train_flat @ x_test_flat.T)   # shape (num_train,num_test)

    dists= train_sq+test_sq-cross
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
    return dists
...
def predict_labels(dists: torch.Tensor, y_train: torch.Tensor, k: int = 1):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    top_dists,top_idnces=torch.topk(dists,k=k,dim=0,largest=False) 
    #print(top_dists,top_dists.shape)
    # top_dists:shape (k,num_test) ,topk_dists: shape (k,num_test)
    # this mean one row of cell  in each is k value
    for i  in range(num_test):       
      votes=top_idnces[:,i]           #(k,)
      vote_labels = y_train[votes]        # get labels of neighbors
      # Count frequency
      values, counts = vote_labels.unique(return_counts=True)
      mx_frq=counts.max()
      # Get all values with max count (could be more than one in case of tie)
      most_commant=values[mx_frq==counts]
      y_pred[i]=most_commant.min()
    #print(y_pred)
    ##########################################################################
    #                           END OF YOUR CODE                             #
    ##########################################################################
...
class KnnClassifier:
    ...
    def predict(self, x_test: torch.Tensor, k: int = 1):
        ...
        ######################################################################
        # Replace "pass" statement with your code
        dists=compute_distances_no_loops(self.x_train,x_test)  # return 2d tensor (num_train,num_test)
        y_test_pred=predict_labels(dists ,self.y_train,k)       #                  (num_test,)
        ######################################################################
        #                         END OF YOUR CODE                           #
        ######################################################################
        return y_test_pred
...
def knn_cross_validate(
    ...
    for f in range(num_folds):
        #test data 
        x_val_fold=x_train_folds[f]
        y_val_flod=y_train_folds[f]
        # train data  in one big tensor
        x_train_fold= torch.cat([  x_train_folds[i] for i in range(num_folds)if i!=f   ])
        y_train_fold=torch.cat([ y_train_folds[i] for i in range(num_folds)if i!=f])
        # calc acc
        for k in k_choices:
          classifier = KnnClassifier(x_train_fold, y_train_fold)
          acc=classifier.check_accuracy(x_val_fold, y_val_flod, k=k,quiet=1)
...
def knn_get_best_k(k_to_accuracies: Dict[int, List]):
    ...
    for  k,value in k_to_accuracies.items():
      mean=torch.mean(torch.tensor(value))
      if best_mean<mean:
        best_k=k
        best_mean=mean
