...
def svm_loss_vectorized(
    ...
    scores = X @ W                         # X shape:(N,D) , W shape:(D,C  ) -->(N,C)
    correct_class_score = scores[torch.arange(N), y]       # shape (N,)
    margin = scores - correct_class_score.view(-1,1) + 1   # note delta = 1  shape(N,c)
    margin[torch.arange(N), y] = 0                         # don't count correct class
    margin = torch.clamp(margin, min=0)                    # same as max(margin, 0)
    loss=margin.sum() /N                                   # scaler
    #print(loss)
    #############################################################################
    #                             END OF YOUR CODE                              #
    #############################################################################
    ...
    #############################################################################
    # Replace "pass" statement with your code
    mask=(margin>0).to(X.dtype)               #(N,c)
    mask_col=mask.sum(dim=1)      #(N,1) 
    mask[torch.arange(N),y] =-mask_col    
...
def sample_batch(
    ...
    #########################################################################
    # Replace "pass" statement with your code
    rand_index = torch.randint(low=0, high=num_train, size=(batch_size,))
...
def train_linear_classifier(
    ...
    for it in range(num_iters):
        ...
        #########################################################################
        # Replace "pass" statement with your code
        W-=learning_rate*grad   #  grad shape (D,C)
        #########################################################################
        #                       END OF YOUR CODE                                #
        #########################################################################
...
def svm_get_search_params():
    ...
    ###########################################################################
    # Replace "pass" statement with your code
    # Learning rates: try a range from small to moderately large
    learning_rates = [1e-3, 3e-3, 1e-2, 3e-2]  #.001 ,.003

    # Regularization strengths: try a range from weak to strong
...
def test_one_param_set(
    ...
    train_acc=100.0*(y_pred_train==y_train).double().mean()
    val_acc=100.0*(y_pred_val==y_val).double().mean()
...
def softmax_loss_naive(
    ...
    num_train=X.shape[0]
    for i in range(num_train):
      score=W.T @ X[i]                       #W shape(D,C) ,x[i] (D,)-->(C,)
      score -= torch.max(score)              # numeric stability trick
      exp_scores = torch.exp(score)
      probs = exp_scores / exp_scores.sum()
      loss+=-torch.log(probs[y[i]])     

      for j in range(num_closs):
        dW[:,j]+= (probs[j]-1*(j==y[i]))*X[i]
...
def softmax_loss_vectorized(
    ...
    scores=X @ W  # X(N,D) ,W(D,C)--->(N,C) ex(1000,10)
    scores -= scores.max(dim=1,keepdim=True).values
    
    exp_scores=torch.exp(scores)
    probs=exp_scores/ exp_scores.sum(dim=1,keepdim=True)    #(N=1000,C=10)/(N=1000,1) -->(N,C)
    probs_idx=probs[ torch.arange(num_train),y]
    loss=-torch.log(probs_idx).sum()
    loss/=num_train
