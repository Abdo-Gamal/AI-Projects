...
def nn_forward_pass(params: Dict[str, torch.Tensor], X: torch.Tensor):
    ...
    hidden_linear= X.mm(W1)+b1           # X(N,D),W1 (D,H) -->(N,H)
    hidden=hidden_linear.clamp(min=0)    #-->(N,H)
    scores=hidden.mm(W2)+b2         #-->o_H1(N,H),W2 (H,C),-->(N,C)
...
def soft_max(x):
    ...
    e = torch.exp(x - torch.max(x, dim=1, keepdim=True).values)  
    return e / e.sum(dim=1, keepdim=True)  # shape: (batch_size, num_classes)
    

def cross_entropy( y_pred,y):
    """
    Cross-entropy when y is a class index.
    y_idx: integer index of correct class
    y_pred: probability vector (after softmax)
    """
    return -torch.log(y_pred[torch.arange(y.shape[0]),y]).sum()

def crossE_with_softmax_grad(probs, y):
    """
    Combined gradient of cross-entropy loss with softmax.
    For the correct class: gradient = p_i - 1
    For incorrect classes: gradient = p_i
    """
    grad = probs.clone()  # Start with softmax probabilities
    # Subtract 1 from the correct class probabilities
    grad[torch.arange(y.shape[0]), y] -= 1
    return grad

def relu_dash(x):
  return (x > 0).float()#torch.where(x<=0,0,1)
...
def nn_forward_backward(
    ...
    scores_softmx=soft_max(scores)# scores(N,c)
    loss=cross_entropy(scores_softmx,y)
    loss/=N
    ...
    grads = {}
    # Step 1: Gradient of loss w.r.t. scores (after softmax)
    # This is the combined gradient of cross-entropy + softmax

    err_upto_softmax =  crossE_with_softmax_grad(scores_softmx, y) # Shape: (N, C)
    err_upto_softmax /= N  # Scale by batch size
    
        # Step 2: Gradients for W2 and b2
    # scores = h1 @ W2 + b2, so:
    # dW2 = h1.T @ dscores
    # db2 = sum(dscores, axis=0)
    grads['W2'] = h1.T @ err_upto_softmax + 2 * reg * W2  # Shape: (H, C)
    grads['b2'] = torch.sum(err_upto_softmax, dim=0)      # Shape: (C,)
    

    # Step 3: Gradient w.r.t. hidden layer output h1
    # h1 = ReLU(X @ W1 + b1), so we need dh1 first
    err_upto_h = err_upto_softmax @ W2.T  # Shape: (N, H)
       # Step 4: Gradient w.r.t. pre-activation (before ReLU)
    # Assuming ReLU activation: gradient is 1 where input > 0, else 0
    # You'll need to store the pre-activation values from forward pass
    # For now, assuming h1_raw is available (you might need to modify forward pass)
    # dh1_raw = dh1 * (h1_raw > 0)  # ReLU derivative

    # Step 5: Gradients for W1 and b1
    # pre_activation = X @ W1 + b1, so:
    # dW1 = X.T @ dh1_raw
    # db1 = sum(dh1_raw, axis=0)
    z1=X @ W1+b1
    dz1=err_upto_h*relu_dash(z1)
    grads['W1']=X.T @ dz1 +2*reg*W1 #X(N,D) ,er(N,H)  -->(D,H)
...
def nn_get_search_params():
    ...
    regularization_strengths=[0,1e-5,1e-4,1e-3]
      ...
      for hs in hidden_sizes:
        for reg in regularization_strengths:
          for lr_decay in learning_rate_decays:
            net = TwoLayerNet(3 * 32 * 32, hs, 10, device=data_dict['y_train'].device, dtype=torch.float32)
